...
import com.cryptoutility.protocol.crypto.{Encrypt, Decrypt, Base64Decode}
...
//  val askUser: () => Future[(String, String, String)] = AskUser.fromConsole
  ...
  lazy val bootStrap = new BootStrap(initialise, askUser, saveUser, extractUserInfo, notifyUser)
  lazy val handler = new ServerHandler(Seq(cryptoService.notify _, bootStrap.notify _), Seq(cryptoService.on _))
  lazy val handlerChain = Server.webSocketHandlers(handler, host, port)(_)
